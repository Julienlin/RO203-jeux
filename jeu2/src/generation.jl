...
function generateInstance(n1::Int64, n2::Int64)
    ...
    G = div(n1*n2,64) +2 #64 est choisi a l'instinct
    g = 1
    while g<=G
    ...
    while !isFilled(N,X)
        ...
        if is4Possible && r<0.25 
            isPossible,C = isAnglePossible(X_empty)
        ...
        elseif !isNode && isVPossible && r<0.5
            isPossible,C = isVerticalPossible(X_empty)
            #S'il est possible de choisir un noyau au sur un coin entre 4 cases
            if isPossible
                isNode = true
                i = rand(1:size(C,1))
        ...
        elseif !isNode && isHPossible && r<0.75
            isPossible,C = isHorizontalPossible(X_empty)
            #S'il est possible de choisir un noyau au sur un coin entre 4 cases
            if isPossible
                isNode = true
                i = rand(1:size(C,1))
